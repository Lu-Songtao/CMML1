...
simulation <- function(Param.df, type, exp_type, save=F, savepath="",
                       ...
                       scale.confi.init,
                       init_mode = c("familiarity", "fixed", "payoff")[1],
                       novel_init_mode = c("same_rule", "low_specific")[1]){
#############################################################
  ## df initialization
  subjectnum = nrow(Param.df)
  simupath = file.path(savepath, paste(type, init_mode, novel_init_mode, sep = "_"))
...
#############################################################    
    ...
    raw_familiarity <- schemainfo$conN
    
    if (init_mode == "familiarity") {
      schemainfo$conN <- 0.2 + 0.8 * raw_familiarity
      
    } else if (init_mode == "fixed") {
      schemainfo$conN <- rep(0.5, nrow(schemainfo))
      
    } else if (init_mode == "payoff") {

      schemainfo$conN <- 0.2 + 0.8 * (
        (schemainfo$payoff - min(schemainfo$payoff)) /
          (max(schemainfo$payoff) - min(schemainfo$payoff))
      )
    }
    
    schemainfo$conVar <- rep(var(schemainfo$conN), nrow(schemainfo))
#############################################################
    ...
    while (clock < 4500){# new clock < 4500 i.e. total time is (85-10)*60 seconds
      ...
      if (break_happens == 0 & clock >= 1500){
        ...
        if (type %in% c("L","H")){
          ...
          raw_new_familiarity <- schemainfo2$conN
          
          if (novel_init_mode == "same_rule") {
            # 新 schema 按旧 schema 同样规则初始化
            if (init_mode == "familiarity") {
              schemainfo2$conN <- 0.2 + 0.8 * raw_new_familiarity
            } else if (init_mode == "fixed") {
              schemainfo2$conN <- rep(0.5, nrow(schemainfo2))
            } else if (init_mode == "payoff") {
              schemainfo2$conN <- 0.2 + 0.8 * (
                (schemainfo2$payoff - min(schemainfo2$payoff)) /
                  (max(schemainfo2$payoff) - min(schemainfo2$payoff))
              )
            }
            
          } else if (novel_init_mode == "low_specific") {
            # 新 schema 一开始 confidence 较低
            schemainfo2$conN <- rep(0.2, nrow(schemainfo2))
          }
          
          schemainfo2$conVar <- rep(var(schemainfo2$conN), nrow(schemainfo2))
          #############################################################
